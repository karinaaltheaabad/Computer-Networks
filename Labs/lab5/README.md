...
I'm not really sure what the answer is, an insight that I have is that maybe since each client can only connect to one server at a time, there should be multiple servers that multiple clients can connect to. This way each client has a different port, and can function synchronously. Another maybe is like in one of the labs, use threading, so that multiple clients can connect to the server sockets. 
