...
class FileManager:
    ...
    def get_block(self, piece_index, offset, length, path='age.txt'):
        """
        gets a block from the file in the path given as parameter
        ...
        :return:
        """
        first_block_byte = (piece_index * length) + piece_index
        block_ends = (piece_index * length) + piece_index + length
        block = first_block_byte + block_ends
        return block

    def get_piece(self, blocks):
        """
        Converts a list of blocks in a piece
